...
class EmissionsModule(BaseModule):
    ...
    def __init__(self,
        ...
        self.emissions_state = np.zeros(shape=(self._cells.yCount,
                                               self._cells.xCount,
                                               len(EmissionType)))
    ...
    def normalize_emission(self, value):
        return (self.mg_to_ug_scaler * value) / (self._cells.cell_height *
                                                 self._cells.cell_width)
    ...
    def get_cell_emissions(self, cell_id, emission_type):
        ...
        return self.emissions_state[cell_obj.matrixPosY, cell_obj.matrixPosX,
                                    emission_type.value]
        ...
        for edge in self._cells.edge_to_cells.keys():
            ...
            for polyId in self._cells.edge_to_cells[edge]:
                for emission_type in self.emission_types:
                    ...
                    emission_value = self.normalize_emission(
                        edge_emissions[self.get_emission_id(
                            emission_type)])  # Multiply by step length
                    x = self._cells.cells[polyId].matrixPosX
                    y = self._cells.cells[polyId].matrixPosY
                    self.emissions_state[y, x,
                                         emission_type.value] += emission_value

        for emission_type in self.emission_types:
            # Apply decay neighbors (Gaussian Filter)
            self.emissions_state[:, :, emission_type.
                                 value] = filters.gaussian_filter(
                                     self.emissions_state[:, :,
                                                          emission_type.value],
                                     EmissionConst.neighbour_decay[str(
                                         emission_type)])

            # Apply decay
            self.emissions_state[:, :, emission_type.
                                 value] = self.emissions_state[:, :,
                                                               emission_type.
                                                               value] * EmissionConst.decay[
                                                                   str(emission_type
                                                                       )]
